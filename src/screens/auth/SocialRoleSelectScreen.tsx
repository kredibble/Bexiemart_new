/**
 * SocialRoleSelectScreen — Role selection with Better Auth.
 *
 * Two entry points:
 * 1. From SocialSignInScreen after OAuth callback → user is authenticated → pick role → updateUser → dashboard
 * 2. From "Sign Up" link → user is not authenticated → pick role → sign up with Google → OAuth callback returns here → updateUser → dashboard
 */
      ...
      >
        ...
        {error && (
          <View style={styles.errorBanner}>
            <Ionicons name="alert-circle" size={16} color="#B3261E" style={{ marginRight: 8 }} />
  ...
  return (
    ...
    >
      <View style={[
        ...
        <Ionicons name={icon as any} size={28} color={isSelected ? '#FFFFFF' : '#8E96A6'} />
      </View>

      <Text style={[styles.roleTileLabel, isSelected && { color: '#111322' }]}>{label}</Text>
      <Text style={[styles.roleTileTagline, isSelected && { color: '#5F6C7B' }]}>{tagline}</Text>
...
const styles = StyleSheet.create({
  ...
  topBar: {
    ...
    paddingHorizontal: 20, position: 'absolute', left: 0, right: 0, zIndex: 10,
  },
  backButton: {
  ...
  roleTile: {
    flex: 1, borderRadius: 24, padding: 20, paddingBottom: 32, alignItems: 'center',
    backgroundColor: '#F8F9FC', borderWidth: 1.5, borderColor: '#E8EBF0',
    ...Platform.select({
